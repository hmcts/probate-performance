...
class Probate_Simulation extends Simulation {
  ...
  val intestacyHourlyTarget:Double = 80
    ...
    .exitBlockOnFail {
      ...
      .doIf(session => session("perc").as[Int] < continueAfterEligibilityPercentage || debugMode != "off" || testType == "pipeline") {
        ...
        .exec(
          Homepage.ProbateHomepage,
          Login.ProbateLogin,
          CitizenHub.ProbateCitizenHub,
          Logout.ProbateLogout)
        .exec(flushHttpCache)
        //Caseworker issues the Probate grant
        .exec(
          CW_IssueGrant.IssueGrant)
    ...
    .exitBlockOnFail {
      exec(_.set("env", s"${env}"))
        .exec(
          ...
          Homepage.ProbateHomepage,
          Login.ProbateLogin,
          Intestacy_01_ExecOne_Apply.IntestacyEligibility)
        .doIf(session => session("perc").as[Int] < continueAfterEligibilityPercentage || debugMode != "off" || testType == "pipeline") {
          exec(
            Intestacy_01_ExecOne_Apply.IntestacyApplicationSection1,
            Intestacy_01_ExecOne_Apply.IntestacyApplicationSection2,
            Intestacy_01_ExecOne_Apply.IntestacyApplicationSection3,
            Logout.ProbateLogout)
            .exec(flushHttpCache)
            .exec(
              Intestacy_02_ExecTwo_Declaration.IntestacyDeclaration)
            .exec(flushHttpCache)
            .exec(
              Homepage.ProbateHomepage,
              Login.ProbateLogin,
              Intestacy_03_ExecOne_Submit.IntestacySubmit,
              Logout.ProbateLogout)
            .exec(flushHttpCache)
            //Simulate the additional traffic generated by the new Citizen Hub feature (half the load is applied here)
            .exec(
              Homepage.ProbateHomepage,
              Login.ProbateLogin,
              CitizenHub.IntestacyCitizenHub,
              Logout.ProbateLogout)
            .exec(flushHttpCache)
            //Caseworker issues the Probate grant
            .exec(
      ...
      case "perftest" | "pipeline" => //currently using the same assertions for a performance test and the pipeline
        if (debugMode == "off") {
          Seq(global.successfulRequests.percent.gte(95),
            details("CCD_SubmitEvent_boIssueGrantForCaseMatching").successfulRequests.percent.gte(80),
